...
[English](README.md) | [当前版本](#当前版本) | [快速开始](#2-分钟启动) | [自动生成程序](#自动生成解决问题的程序) | [远程访问](#远程与-vpn-访问) | [当前限制](#当前-alpha-限制)
...
当前公开 Release：`v0.1.2-alpha`<br>
package version：`0.1.2-alpha.0`

这里仅列出已经完成、已有测试或发布检查覆盖的能力。

| 版本 | 已完成能力 |
| --- | --- |
| `0.1.2-alpha.0` | Docker Compose 本地 Markdown 演示、GHCR 镜像路径、macOS unsigned ZIP、Windows NSIS 安装包、Linux AppImage、管理员认证、AI provider 设置、手机 PWA 绑定、离线队列、SQLite migration、备份恢复、诊断包、发布检查、连接诊断、Studio 生成程序运行日志、调试指令和一键修复保存。 |
| `0.1.1-alpha.0` | Docker quickstart、Ollama、本地 Markdown vault 读取、quickstart 登录、GHCR workflow、本地模型默认配置。 |
| `0.1.0` | 初始桌面/PWA 底座：管理员安全、设备绑定、SQLite 数据、备份恢复、AI provider 配置、URL Scheme 安全、桌面打包脚本。 |
...
| 自动生成程序 | 已实现 Studio 生成、改写、运行日志、调试指令、一键修复保存、状态存储和回滚 |
...
`0.1.2-alpha.0` 状态：已实现生成、手动改写、持久状态、运行日志、调试指令生成、一键修复保存、动作权限检查和版本回滚。无人值守的完全自动自修复闭环还没有发布，所以这里不作为功能宣传。
