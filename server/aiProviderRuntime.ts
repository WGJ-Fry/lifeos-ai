...
function baseUrlForProvider(providerId: AiProviderId, credential: string) {
  if (providerId === "openrouter") return "https://openrouter.ai/api/v1";
  if (providerId === "local") return credential.replace(/\/$/, "");
...
async function generateOpenAiCompatible(input: GenerateAiContentInput, credential: string, providerId: AiProviderId, model: string): Promise<AiProviderResponse> {
  ...
  const response = await fetch(`${baseUrlForProvider(providerId, credential)}/chat/completions`, {
    method: "POST",
    headers: {
