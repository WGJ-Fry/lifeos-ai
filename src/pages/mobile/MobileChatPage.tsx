    ...
    return (
      <div className="min-h-screen bg-[#060a10] text-zinc-100 px-5 py-8 flex items-center justify-center">
        <div className="w-full max-w-sm">
          ...
          <h1 className="text-2xl font-bold tracking-tight">LifeOS 手机端</h1>
          <p className="mt-3 text-sm leading-relaxed text-zinc-400">
            {recoveringPairingIntent
              ? "正在恢复添加到桌面时保存的绑定信息，稍后会自动回到确认绑定页。"
              : "这台手机还没有绑定电脑端。请在电脑管理端生成二维码，用手机扫码完成绑定。"}
          </p>
          ...
          <div className="mt-7 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
            <div className="flex gap-3">
              ...
              <div>
                <div className="text-sm font-bold">绑定后会保存设备凭证</div>
                <p className="mt-1 text-xs leading-relaxed text-zinc-500">
                  之后打开手机端会自动连接这台电脑，电脑端可以随时撤销访问权限。
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
            <label className="text-xs font-bold text-zinc-500">没有扫成功时，粘贴电脑端绑定链接</label>
            <input
              value={pairingInput}
              onChange={(event) => {
        ...
        <div className="fixed bottom-4 left-3 right-3 z-[100] mx-auto max-w-md rounded-2xl border border-white/[0.12] bg-[#101722]/95 p-3 text-xs text-zinc-200 shadow-2xl backdrop-blur-xl">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="font-bold text-zinc-100">添加到主屏幕</div>
              <div className="mt-1 leading-relaxed text-zinc-400">
                {installPrompt ? "安装后会像普通手机 App 一样打开 LifeOS。" : "在浏览器分享或菜单里选择添加到主屏幕，之后可直接打开 LifeOS。"}
              </div>
              {installPrompt ? (
            ...
            <button aria-label="关闭安装提示" onClick={dismissInstallHint} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-zinc-400">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
