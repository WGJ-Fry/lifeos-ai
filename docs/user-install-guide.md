...
OwnOrbit AI 是电脑端应用 + 手机端 PWA。电脑端负责运行本地核心、保存数据、连接 AI；手机端通过浏览器扫码绑定后使用。

## 先看这里：当前公开版本状态

| 项目 | 当前状态 | 用户应该怎么做 |
| --- | --- | --- |
| Docker Compose alpha | 已使用 `ghcr.io/wgj-fry/lifeos-ai:v0.1.5-alpha` | 推荐第一次体验使用。先确认 GitHub Packages 页面已经公开，并用 `docker pull ghcr.io/wgj-fry/lifeos-ai:v0.1.5-alpha` 验证能拉取。 |
| macOS 桌面包 | `v0.1.5-alpha` 已上传 `LifeOS.AI-0.1.5-alpha.0-arm64-unsigned.zip` | 适合早期桌面测试。它是 unsigned ZIP，不是签名公证 DMG。 |
| Windows 桌面包 | `v0.1.5-alpha` 已上传 `LifeOS.AI.Setup.0.1.5-alpha.0.exe` | 适合早期桌面测试。当前 EXE 未做正式 Authenticode 签名，SmartScreen 可能提示未知发布者。 |
| Linux 桌面包 | `v0.1.5-alpha` 已上传 `LifeOS.AI-0.1.5-alpha.0.AppImage` | 适合早期桌面测试。运行前请赋予可执行权限，并按 `SHA256SUMS` 校验。 |
| 自动更新 | 当前未启用 | 先按手动下载、校验 SHA256、覆盖安装的方式更新。 |

请使用明确的 `v0.1.5-alpha` Release 链接，不要依赖 GitHub 通用的 Latest release 标签；旧的 `v0.1.0` / `v0.0.0` 只应作为历史记录保留。
...
1. 下载 unsigned ZIP。
2. 解压 ZIP。
3. 把 `OwnOrbit AI.app` 拖到 Applications。
4. 从 Applications 打开。

当前 `v0.1.5-alpha` 公开 macOS 资产是 unsigned ZIP，不是已签名公证 DMG。macOS 可能提示 Gatekeeper，需要按 Release 里的 `INSTALL-unsigned-mac.md` 继续打开。后续如果发布带签名环境构建的 macOS DMG，它会是 Developer ID 签名、Apple 公证、并完成 stapled。

如果你习惯用终端复制 `.app`，优先使用 `ditto`，不要用 `cp -R`。`ditto` 更接近 Finder 拖拽安装，能更稳定地保留 macOS 对签名应用需要的元数据：

```bash
ditto "/path/to/OwnOrbit AI.app" "/Applications/OwnOrbit AI.app"
```
...
```bash
chmod +x "LifeOS.AI-0.1.5-alpha.0.AppImage"
./"LifeOS.AI-0.1.5-alpha.0.AppImage"
```
...
如果首次启动时桌面窗口没有正常打开，但本地核心已经启动，不用重新安装。桌面失败页会提供 `Retry OwnOrbit AI`、`Open Local Console In Browser`、`Copy Local Address`、`Open Logs Folder`、`Copy Logs Path`、`Export Desktop Diagnostics`。这时可以先点 `Open Local Console In Browser`，继续完成管理员设置、AI Key、手机绑定和第一次聊天，再回头导出日志或诊断包排查桌面壳问题。
...
不要直接把 OwnOrbit AI 暴露到公网 IP。开启 LAN/公网模式前，请确认管理员密码已设置，并在连接向导里查看安全提示。
...
11. 如果你是 Apple 设备优先使用 iCloud 入口，在首次流程的“Apple / iCloud 异地入口”卡片里也可以直接记录这些真实设备测试。每一项都需要写清手机、网络、入口和结果；OwnOrbit 会把它写入同一套长期异地验收记录和诊断包。
...
16. 退出并重新打开电脑端 OwnOrbit AI，再运行“立即检查异地健康”；确认恢复后，在同一清单记录“重启后自动恢复”。
...
`trycloudflare.com` 快速隧道是临时地址。OwnOrbit AI 会在下次启动时自动重新创建 Tunnel 并刷新二维码地址，但已经添加到手机主屏幕的旧临时域名可能失效。需要长期固定入口时，请使用 Tailscale、Cloudflare Named Tunnel，或自己的可信 HTTPS 反向代理。
...
```bash
shasum -a 256 "LifeOS.AI-0.1.5-alpha.0-arm64-unsigned.zip"
shasum -a 256 "LifeOS.AI-0.1.5-alpha.0.AppImage"
```
...
```powershell
Get-FileHash ".\LifeOS.AI.Setup.0.1.5-alpha.0.exe" -Algorithm SHA256
```
...
自动更新以后需要配置安全的 HTTPS 目录 `LIFEOS_UPDATE_URL`，发布 `release-manifest.json` 和对应 `latest*.yml`。signed 分发可以在安全 feed 下默认进入更新检查；unsigned alpha 仍需要显式设置 `LIFEOS_ENABLE_DESKTOP_AUTO_UPDATE=1`，否则继续走手动下载和 SHA256 校验。
...
OwnOrbit AI is a desktop app plus a mobile PWA. The desktop app runs the local core, stores data, and connects to AI providers. The phone connects through a paired browser/PWA.

## Read This First: Current Public Release Status

| Item | Current status | What users should do |
| --- | --- | --- |
| Docker Compose alpha | Uses `ghcr.io/wgj-fry/lifeos-ai:v0.1.5-alpha` | Recommended first try. Confirm the GitHub Packages page is public and verify `docker pull ghcr.io/wgj-fry/lifeos-ai:v0.1.5-alpha` works before promoting it. |
| macOS desktop package | `v0.1.5-alpha` uploads `LifeOS.AI-0.1.5-alpha.0-arm64-unsigned.zip` | Good for early desktop testing. It is an unsigned ZIP, not a signed and notarized DMG. |
| Windows desktop package | `v0.1.5-alpha` uploads `LifeOS.AI.Setup.0.1.5-alpha.0.exe` | Good for early desktop testing. It is not Authenticode signed yet, so SmartScreen may warn about an unknown publisher. |
| Linux desktop package | `v0.1.5-alpha` uploads `LifeOS.AI-0.1.5-alpha.0.AppImage` | Good for early desktop testing. Mark it executable and verify it with `SHA256SUMS`. |
| Auto-update | Not enabled yet | Update manually by downloading the new build, verifying SHA256, and installing it. |

Use the explicit `v0.1.5-alpha` Release link instead of relying on GitHub's generic Latest release label; older `v0.1.0` / `v0.0.0` releases should remain historical only.
...
5. Open OwnOrbit AI from the Start Menu or desktop shortcut.
...
8. After finishing the first launch guide, OwnOrbit AI now opens the first chat directly. Send one test message right away to confirm the AI provider, local core, and chat path are working.
...
Do not expose OwnOrbit AI directly to a public IP. Before enabling LAN/public mode, set an admin password and review the security hints in the connection guide.
...
7. If one-click start is unavailable, run the command shown by OwnOrbit AI, for example:
...
2. Open the OwnOrbit AI desktop connection guide.
...
Model IDs are not locked to the built-in list. The desktop UI shows common versions, refreshes `/models` when the provider supports it, and still lets you type a newly released model ID manually.
...
Future auto-update requires a safe HTTPS directory in `LIFEOS_UPDATE_URL`, `release-manifest.json`, and the matching `latest*.yml` feed files. Signed distributions can enter update checks by default with a safe feed; unsigned alpha builds still require the explicit `LIFEOS_ENABLE_DESKTOP_AUTO_UPDATE=1` opt-in, otherwise they stay on manual download plus SHA256 verification.
