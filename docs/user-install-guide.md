...
| 项目 | 当前状态 | 用户应该怎么做 |
| --- | --- | --- |
| Docker Compose alpha | 已使用 `ghcr.io/wgj-fry/lifeos-ai:v0.1.4-alpha` | 推荐第一次体验使用。先确认 GitHub Packages 页面已经公开，并用 `docker pull ghcr.io/wgj-fry/lifeos-ai:v0.1.4-alpha` 验证能拉取。 |
| macOS 桌面包 | `v0.1.4-alpha` 已上传 `LifeOS.AI-0.1.4-alpha.0-arm64-unsigned.zip` | 适合早期桌面测试。它是 unsigned ZIP，不是签名公证 DMG。 |
| Windows 桌面包 | `v0.1.4-alpha` 已上传 `LifeOS.AI.Setup.0.1.4-alpha.0.exe` | 适合早期桌面测试。当前 EXE 未做正式 Authenticode 签名，SmartScreen 可能提示未知发布者。 |
| Linux 桌面包 | `v0.1.4-alpha` 已上传 `LifeOS.AI-0.1.4-alpha.0.AppImage` | 适合早期桌面测试。运行前请赋予可执行权限，并按 `SHA256SUMS` 校验。 |
| 自动更新 | 当前未启用 | 先按手动下载、校验 SHA256、覆盖安装的方式更新。 |

请使用明确的 `v0.1.4-alpha` Release 链接，不要依赖 GitHub 通用的 Latest release 标签；旧的 `v0.1.0` / `v0.0.0` 只应作为历史记录保留。
...
```text
LifeOS.AI-0.1.4-alpha.0-arm64-unsigned.zip
```
...
当前 `v0.1.4-alpha` 公开 macOS 资产是 unsigned ZIP，不是已签名公证 DMG。macOS 可能提示 Gatekeeper，需要按 Release 里的 `INSTALL-unsigned-mac.md` 继续打开。后续如果发布带签名环境构建的 macOS DMG，它会是 Developer ID 签名、Apple 公证、并完成 stapled。
...
```text
LifeOS.AI.Setup.0.1.4-alpha.0.exe
```
...
```text
LifeOS.AI-0.1.4-alpha.0.AppImage
```

运行：

```bash
chmod +x "LifeOS.AI-0.1.4-alpha.0.AppImage"
./"LifeOS.AI-0.1.4-alpha.0.AppImage"
```
...
```bash
shasum -a 256 "LifeOS.AI-0.1.4-alpha.0-arm64-unsigned.zip"
shasum -a 256 "LifeOS.AI-0.1.4-alpha.0.AppImage"
```
...
```powershell
Get-FileHash ".\LifeOS.AI.Setup.0.1.4-alpha.0.exe" -Algorithm SHA256
```
...
| --- | --- | --- |
| Docker Compose alpha | Uses `ghcr.io/wgj-fry/lifeos-ai:v0.1.4-alpha` | Recommended first try. Confirm the GitHub Packages page is public and verify `docker pull ghcr.io/wgj-fry/lifeos-ai:v0.1.4-alpha` works before promoting it. |
| macOS desktop package | `v0.1.4-alpha` uploads `LifeOS.AI-0.1.4-alpha.0-arm64-unsigned.zip` | Good for early desktop testing. It is an unsigned ZIP, not a signed and notarized DMG. |
| Windows desktop package | `v0.1.4-alpha` uploads `LifeOS.AI.Setup.0.1.4-alpha.0.exe` | Good for early desktop testing. It is not Authenticode signed yet, so SmartScreen may warn about an unknown publisher. |
| Linux desktop package | `v0.1.4-alpha` uploads `LifeOS.AI-0.1.4-alpha.0.AppImage` | Good for early desktop testing. Mark it executable and verify it with `SHA256SUMS`. |
| Auto-update | Not enabled yet | Update manually by downloading the new build, verifying SHA256, and installing it. |

Use the explicit `v0.1.4-alpha` Release link instead of relying on GitHub's generic Latest release label; older `v0.1.0` / `v0.0.0` releases should remain historical only.
