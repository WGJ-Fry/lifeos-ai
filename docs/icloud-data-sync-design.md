...
That report records the helper probe, disposable roundtrip, subscription probe, and every required real Apple-device manual check. A passing helper pack is still only `automated-ready-manual-required`; public claims require current evidence for Mac-to-Mac import, native iPhone CloudKit read/write-back, iPhone push or BGAppRefresh recovery, cellular entry behavior, Wi-Fi/cellular switching, Mac restart recovery, iCloud delay copy, old entry/QR expiry, multi-desktop defaults, native background push delivery, and offline conflict review.
...
- an expired desktop change token restarts a full baseline, marks every recovered record for review, and blocks silent SQLite overwrite or checkpoint promotion until that review is resolved;
...
当前还新增了源码级 iOS SwiftUI 原生壳候选版本：`native/apple/mobile-shell`。它可以通过 iPhone“文件”选择 iCloud Drive 中的 `lifeos-mobile-entry-*.json`，校验与电脑端一致的 SHA-256、版本、有效期、地址来源和 OwnOrbit 健康接口，然后在限制同源跳转的 `WKWebView` 中打开 `/mobile/chat`。它也加入了用户明确开启的 CloudKit 私有库增量拉取、变更游标、私有数据库推送订阅、非 CloudKit/非私有库通知过滤、前台恢复、账号隔离、游标过期 zone 重建、自动续页与退避重试、记录 schema/zone/type/大小/SHA-256 校验和 Data Protection 离线快照。开启 CloudKit 后还会提交最早 30 分钟后可运行的 `BGAppRefreshTask` 兜底任务并在唤醒后续约；它是系统择机调度的恢复入口，不是固定 30 分钟轮询。当前已形成两条真实双向写回候选路径：用户确认后，iPhone 可以新建一条通过敏感内容检测的普通记忆，Mac 仅在记录全新时写入 SQLite；也可以把一条未完成任务改为完成，并通过 CKRecord change tag 与基础内容哈希做双重乐观并发。已有记忆、聊天、生成程序和设备元数据仍只读。`npm run mobile:native:build`、`LIFEOS_IOS_NATIVE_RUN_TESTS=1 npm run mobile:native:build` 和 `npm run mobile:native:smoke -- http://127.0.0.1:3000` 可以在未签名的 iPhone 模拟器完成构建、单测、安装、启动和截图证据；`npm run mobile:native:device:build` 提供带 CloudKit entitlement 的真机签名入口。模拟器能证明后台任务声明、标识符边界、明确开启策略和任务完成保护，但不能证明 iOS 真机一定执行后台刷新。它还不是公开 iOS 安装包；在 Apple 协议、Container、provisioning、系统实际后台调度和真实双设备数据往返证据完成前，也不能宣称 CloudKit 已真实跑通。
