...
`v0.1.4-alpha` GitHub Release 草稿必须上传下面这些文件。发布前请先确认 GitHub Actions 产物或本地重新打包产物真实存在：
...
```text
LifeOS AI-0.1.4-alpha.0-arm64.dmg
...
```text
请以 `v0.1.4-alpha` Release 草稿里的 `SHA256SUMS` 为准。
...
5. Windows/Linux 下载说明指向真实存在的 `v0.1.4-alpha` 资产。
...
The `v0.1.4-alpha` GitHub Release draft must upload the following files. Before publishing, confirm the GitHub Actions artifacts or locally rebuilt artifacts actually exist:
...
```text
Use the `SHA256SUMS` file attached to the `v0.1.4-alpha` Release draft.
...
5. Windows/Linux download instructions point to real `v0.1.4-alpha` assets.
