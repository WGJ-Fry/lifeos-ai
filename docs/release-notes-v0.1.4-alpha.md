...
- Applied Studio auto-repairs remain visible until a smoke check is recorded as passed or failed. Low-risk completions can also request a conservative server-side static smoke gate that records pass/fail evidence automatically; failed checks recommend rollback and keep the repair in review instead of allowing another unattended pass.
- Failed server-side static smoke reviews can now trigger an automatic rollback to the recorded safe version for low-risk Studio auto-repairs, keeping unsafe repairs from remaining active.
...
- Calendar/task acceptance runs now expose a two-way evidence gate: external read, guarded write, rollback evidence, connector readiness, and conflict review must all pass before an acceptance run can be marked completed.
...
- 后端静态烟测失败时，低风险 Studio 自动修复现在可以自动回滚到记录的安全版本，避免不安全修复继续处于活动状态。
...
- 日历/任务验收运行现在有双向证据门槛：外部读取、受控写入、回滚证据、连接器就绪和冲突复核都通过后，验收运行才会标记为完成。
