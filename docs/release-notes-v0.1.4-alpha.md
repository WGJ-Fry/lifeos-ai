...
- Calendar/task sync preview safety gates: `.ics` support remains read-only, while Apple Calendar, Google Calendar, and system reminders write-back stay blocked dry-run operations until real connectors ship.
...
- 日历/任务同步预览安全闸门：`.ics` 仍然只是本地只读读取；Apple Calendar、Google Calendar、系统提醒事项写回在真实连接器发布前保持 blocked dry-run。
