...
当前 `v0.1.2-alpha` 已上传 Windows NSIS 安装包，但还没有 Authenticode 正式签名，可能会出现 SmartScreen 未知发布者提示。macOS 当前公开包是 Apple Silicon unsigned ZIP，不是正式签名公证版；Linux 当前公开包是 AppImage。请只从 GitHub Release 下载，并用 `SHA256SUMS` 校验文件。
