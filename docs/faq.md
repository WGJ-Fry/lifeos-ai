...
当前公开版本默认走手动下载更新。仓库已经生成 electron-updater feed 文件，但需要配置稳定 HTTPS 更新地址 `LIFEOS_UPDATE_URL`，并显式设置 `LIFEOS_ENABLE_DESKTOP_AUTO_UPDATE=1` 后才会启用更新检查。
...
The current public release uses manual updates. The repo can generate electron-updater feed files, but automatic update checks require a stable HTTPS `LIFEOS_UPDATE_URL` plus the explicit `LIFEOS_ENABLE_DESKTOP_AUTO_UPDATE=1` opt-in.
