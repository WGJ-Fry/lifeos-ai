...
发布 draft 后，跑一次公开入口检查。它会确认 GHCR 镜像不用登录也能拉取，并确认 `v0.1.4-alpha` Release 已经公开可见：
...
4. 把 `v0.0.0` 标记为 deprecated / 已废弃，并引导用户使用 `v0.1.4-alpha`。
5. 更新 `v0.1.4-alpha` Release 正文，让 Release 页面本身也能独立说明下载、安装、校验和 unsigned alpha 限制。
...
- Windows x64 NSIS installer: `LifeOS.AI.Setup.0.1.4-alpha.0.exe`
...
Linux: download the AppImage, run `chmod +x "LifeOS.AI-0.1.4-alpha.0.AppImage"`, then launch it.
...
```text
Use the SHA256SUMS generated for the current v0.1.4-alpha Release draft.
...
- 版本号已升级为 `0.1.4-alpha.0`；后续更稳定后可再考虑 `0.2.0-beta.1`、`1.0.0-beta.1` 或 `1.0.0`。
...
After publishing the draft, run the public launch check. It confirms the GHCR image is anonymously pullable and that the `v0.1.4-alpha` Release is publicly visible:
...
4. Mark `v0.0.0` as deprecated and point users to `v0.1.4-alpha`.
5. Refresh the `v0.1.4-alpha` Release body so the Release page itself explains downloads, install, verification, and unsigned alpha limits.
...
- `v0.1.4-alpha` is the recommended public entry.
...
- Version is now `0.1.4-alpha.0`; consider `0.2.0-beta.1`, `1.0.0-beta.1`, or `1.0.0` once the project is ready for a broader stable launch.
