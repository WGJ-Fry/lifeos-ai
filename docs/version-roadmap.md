...
| Calendar and tasks | Local `.ics` ingestion is read-only. Apple Calendar and system reminders can be read as opt-in macOS connector previews; Apple Calendar create/update/delete and Reminders create/update/complete/delete require explicit admin confirmation, rollback guidance, and audit logging; Google Calendar two-way sync is not shipped. | LifeOS can read exported/local calendar data and opt-in macOS previews, and can perform narrowly gated macOS writes when explicitly enabled, but it is not a full account sync system yet. |
...
| 日历和任务 | 本地 `.ics` 仍是只读读取。Apple Calendar 和系统提醒事项可以作为显式开启的 macOS connector 外部读取预览；Apple Calendar create/update/delete 和 Reminders create/update/complete/delete 需要管理员确认、回滚提示和审计日志；Google Calendar 双向同步还没发布。 | 可以读导出的/本地日历数据和 macOS 外部预览，也可以在非常窄的授权路径里执行 macOS 写入，但还不是完整账号同步系统。 |
