...
| 日历和任务 | 本地 `.ics` 仍是只读读取。Apple Calendar 和系统提醒事项可以作为显式开启的 macOS connector 外部读取预览；外部写入只在管理员确认后可用；Google Calendar 双向同步还没发布。 | 可以读导出的/本地日历数据和 macOS 外部预览，也可以在非常窄的授权路径里执行 macOS 写入，但还不是完整账号同步系统。 |
