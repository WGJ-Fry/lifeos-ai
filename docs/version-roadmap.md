...
- Calendar/task sync safety gates with read-only `.ics` ingestion plus an opt-in macOS Apple Calendar/System Reminders connector path for explicitly confirmed external writes; Google Calendar remains preview-only until OAuth ships.
...
| Calendar and tasks | Local `.ics` ingestion is read-only. Apple Calendar and system reminders external writes exist only through the opt-in macOS connector path with explicit admin confirmation; Google Calendar two-way sync is not shipped. | LifeOS can read exported/local calendar data and can perform narrowly gated macOS writes when explicitly enabled, but it is not a full account sync system yet. |
...
- 日历/任务同步安全闸门：本地 `.ics` 只读读取，并提供显式开启的 macOS Apple Calendar / 系统提醒事项连接器路径；Google Calendar 仍等 OAuth。
...
| 日历和任务 | 本地 `.ics` 仍是只读读取。Apple Calendar 和系统提醒事项外部写入只在显式开启的 macOS connector 路径里可用，并要求管理员确认；Google Calendar 双向同步还没发布。 | 可以读导出的/本地日历数据，也可以在非常窄的授权路径里执行 macOS 写入，但还不是完整账号同步系统。 |
