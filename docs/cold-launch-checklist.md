# LifeOS v0.1.4-alpha Cold Launch Checklist
...
- [ ] `package.json` is `0.1.4-alpha.0`, `private: false`, and `MIT`.
...
- [ ] `docker-compose.yml` points to `ghcr.io/wgj-fry/lifeos-ai:v0.1.4-alpha`.
...
```bash
docker build -t ghcr.io/wgj-fry/lifeos-ai:v0.1.4-alpha .
...
```text
Tag: v0.1.4-alpha
Title: v0.1.4-alpha: Ask "What am I forgetting?" from local Markdown notes
Body: docs/release-notes-v0.1.4-alpha.md
```
...
If this fails with `GitHub public Release is not visible`, publish the generated Release draft or create the `v0.1.4-alpha` Release before sharing the README.
