...
当你配置 Gemini、OpenAI、OpenRouter 或本地模型时，聊天内容会按对应 provider 的方式发送到该服务。请自行阅读所选 AI provider 的隐私政策和数据处理条款。
...
When Gemini, OpenAI, OpenRouter, or another provider is configured, chat content is sent to that provider according to its own terms. Review the provider's privacy policy before use.
