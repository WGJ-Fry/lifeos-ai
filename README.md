...
1. **电脑端做私有 AI 核心**：连接 Gemini、OpenAI、OpenRouter 或本地模型，保存 SQLite 数据、备份、设备和安全设置。
2. **手机端做随身 AI 管家**：扫码绑定后作为 PWA 使用，聊天、离线队列、设备状态和本地动作确认都在手机上完成。
3. **跨网络保持连接**：同 Wi-Fi 用 LAN，异地优先用 Tailscale、Cloudflare Tunnel 或可信 HTTPS 反向代理。

5 分钟开始使用：下载电脑端，设置管理员密码，配置 AI Key，手机扫码绑定，然后开始聊天或生成自己的小工具。详见 [快速开始](docs/quick-start.md)。

## 适合谁

| 适合 | 暂不适合 |
| --- | --- |
| 想把 AI 数据和密钥留在自己电脑上的用户 | 想要纯云端免配置服务的用户 |
| 想用手机访问自己电脑 AI 的用户 | 想要 App Store 原生手机 App 的用户 |
| 想本地保存聊天、记忆、备份和审计的用户 | 不愿意配置任何 AI Key 或网络连接的用户 |
| 想用 AI 生成个人小工具/微应用的用户 | 需要企业级多人权限和团队协作后台的用户 |
...
LifeOS AI does three things:

1. **Desktop private AI core**: connect Gemini, OpenAI, OpenRouter, or local models while keeping SQLite data, backups, devices, and security settings on your computer.
2. **Mobile personal AI companion**: pair your phone as a PWA for chat, offline queue, device state, and local-action confirmations.
3. **Remote connectivity**: use LAN on the same Wi-Fi, or Tailscale, Cloudflare Tunnel, or a trusted HTTPS reverse proxy away from home.

Start in five minutes: download the desktop app, set an admin password, configure an AI key, pair your phone, then chat or generate your own small tools. See [Quick Start](docs/quick-start.md).

## Who It Is For

| Good fit | Not ideal yet |
| --- | --- |
| People who want AI data and keys to stay on their own desktop | People who want a zero-configuration cloud service |
| People who want phone access to their own desktop AI core | People who require a native App Store mobile app |
| People who want local chats, memories, backups, and audit logs | People who do not want to configure any AI key or network access |
| People who want AI-generated personal tools and micro apps | Teams needing enterprise multi-user administration |
