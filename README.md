...
LifeOS AI 是一个所有人都可以长期自用的个人 AI 管家，它把电脑、手机、AI、网络和本地能力连成一个系统：

1. **电脑端私有 AI 核心**：连接 Gemini、OpenAI、OpenRouter 或本地模型，统一保存 SQLite 数据、备份、设备和安全设置。
2. **手机端随身 AI 管家**：扫码绑定后作为 PWA 使用，聊天、离线队列、设备状态和动作确认都在手机上完成。
3. **VPN/隧道异地连接**：同 Wi-Fi 用 LAN，不在同一局域网时用 Tailscale、Cloudflare Tunnel 或可信 HTTPS 反向代理。
4. **自动生成需要的程序**：在 Studio 里描述想法，AI 自动生成可运行的离线微应用/小程序，并支持继续调试 HTML/CSS/JS。
...
LifeOS AI is a personal AI assistant designed for long-term everyday use. It connects your desktop, phone, AI providers, network access, and local capabilities into one system:
...
3. **VPN/tunnel remote access**: use LAN on the same Wi-Fi, or Tailscale, Cloudflare Tunnel, or a trusted HTTPS reverse proxy away from home.
4. **AI-generated apps and tools**: describe an idea in Studio, generate a runnable offline micro app, then keep refining HTML/CSS/JS.
