...
- A signed native CloudKit chat outbox. The iPhone registers a scoped P-256 public key as `LifeOSDeviceKey`, keeps the private key in device-only Keychain storage, signs each `LifeOSChatRequest`, and renders the matching `LifeOSChatResponse` with waiting, Mac unavailable, processing, retrying, completed, failed, and timeout states. After a terminal response is cryptographically verified and stored in the protected phone snapshot, the iPhone uploads a deterministic signed `LifeOSChatReceipt`; the Mac keeps the SQLite job until that receipt is verified against the still-active paired device. The Mac worker is text-only and rejects tool/native-action requests.
