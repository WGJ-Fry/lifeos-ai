...
test("Electron desktop exports a redacted desktop diagnostic bundle", async (t) => {
  ...
  assert.equal(diagnostics.updates.mode, "blocked");
...
test("Electron desktop keeps a startup failure window open when local core fails", async (t) => {
  ...
  assert.match(desktopMain, /Notification/);
  assert.match(desktopMain, /maybeNotifyDesktopStatus/);
  ...
  assert.match(desktopMain, /resolvePreferredDesktopUserDataPath/);
  assert.match(desktopMain, /getOwnOrbitIcloudFolder/);
  const desktopUserDataPath = await readFile(path.join(rootDir, "desktop", "userDataPath.cjs"), "utf8");
  assert.match(desktopUserDataPath, /lifeos-ai/);
  assert.match(desktopUserDataPath, /LifeOS AI/);
  ...
  assert.match(desktopPreload, /openIcloudSettings/);
});
