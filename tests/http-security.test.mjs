...
test("API error response redaction removes secrets without changing business tokens", () => {
  const payload = {
    error: "Failed with Basic Z2l0aHViOmVycm9y github_pat_errorSecret_1234567890 /Users/local-private/private.txt",
    ...
    details: [
      { detail: "C:\\Users\\local-private\\secret.txt" },
  ...
  assert.doesNotMatch(serialized, /Z2l0aHViOmVycm9y|github_pat_errorSecret|admin-error-token|user:pass|token=secret|#debug|local-private|sk-error-secret|AIzaSy-error|ghp_errorSecret/);
});
